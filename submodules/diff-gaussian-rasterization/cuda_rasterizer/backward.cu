...
void computeCov2DCUDA(int P,
	...
	float* dL_dcov)
{
	for (auto idx = 0; idx < P && radii[idx] > 0; idx++) {
...
void computeCov3D(int idx, const glm::vec3 scale, float mod, const glm::vec4 rot, const float* dL_dcov3Ds, glm::vec3* dL_dscales, glm::vec4* dL_drots)
{
